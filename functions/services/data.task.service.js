...
class DataTask extends EventEmitter {
    ...
    constructor() {
        ...
        this.listeners = [
            ...
            this.on(
                ...
                (_, country, count) =>
                    count <= 4 &&
                    this.runCountryTask(country, this, true, count)
            ),
            this.on(
                "rescheduleFullDump",
                (_, country, count) =>
                    count <= 4 &&
                    this.runCountryTotalDump(country, this, true, count)
            ),
    ...
    async runCountryTask(country, self, rescheduled, count = 0) {
        if (rescheduled) {
            count++;
            console.log(
                `Reschedule ${country.Country} for the ${count}th time`
            );
        }
        ...
        await getCountryData(country.Slug)
            .then(async (res) => {
                if (res.data) {
                    ...
                    throw new Error("No data contained in successful response");
            })
            .catch((err) =>
                self.emit("rescheduleFullDump", err, country, count)
            );
    }
}
